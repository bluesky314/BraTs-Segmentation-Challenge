...
train_size=6   # Set to None to use all     # DataLoader: train size
...
class UNet(torch.nn.Module):        # input -> shrunk in 1/2 then transposed at end. Loss can be with orig seg label / shurken if remove transpose
    def __init__(self):
        ...
        self.down_block4 = UNet_down_block(524, 1024, True)
        self.max_pool = torch.nn.MaxPool3d(2, 2)
        ...
        self.up_block1 = UNet_up_block(1024, 1024, 524)
...
#                     print('lb3',b3.size(),sup_block3.size())
                    ...
                    if e=='train': 
                        ...
                        if epoch>120 and epoch%15==0: # save every 15- for logs- confilicting with down
